#!/usr/bin/env python3
# setup_v2.py - Ejecuta desde la raiz del proyecto: python3 setup_v2.py
...
function AddRecipeModal({open,onClose,onAdd,apiKey,onNeedKey}){
  ...
  const TPL='{"title":"TITULO","description":"DESCRIPCION breve","image":"URL imagen del plato si existe o vacio","mealType":"Comida","recipeType":"Otros platos","ingredients":[{"amount":"200","unit":"gramos","name":"ingrediente"}],"steps":["paso uno","paso dos"],"sourceUrl":"","time":"30 min","servings":4}';
  ...
  async function importRecipe(prompt,imgData=null){
    ...
    try{
      ...
      onAdd({id:Date.now(),title:data.title||"Receta",description:data.description||"",image:data.image||"",mealType:MEAL_TYPES.includes(data.mealType)?data.mealType:"Comida",recipeType:RECIPE_TYPES.includes(data.recipeType)?data.recipeType:"Otros platos",ingredients:(data.ingredients||[]).map((ing,i)=>({id:Date.now()+i,amount:String(ing.amount||""),unit:String(ing.unit||""),name:String(ing.name||"")})),steps:Array.isArray(data.steps)?data.steps:data.steps?data.steps.split("\n").filter(s=>s.trim()):[],sourceUrl:data.sourceUrl||"",time:data.time||"",servings:Number(data.servings)||4,rating:0});
      onClose();
  ...
  return(
    <div style={{background:"#fff",borderRadius:16,overflow:"hidden",boxShadow:"0 2px 12px rgba(0,0,0,.07)",cursor:"pointer",position:"relative",transition:"transform .15s,box-shadow .15s"}}
      ...
      <div style={{position:"relative",height:180,background:"#F3F4F6",overflow:"hidden"}} onClick={()=>onOpen(recipe)}>
        {recipe.image?<img src={recipe.image} alt={recipe.title} style={{width:"100%",height:"100%",objectFit:"cover"}} onError={e=>{e.target.style.display="none"}}/>:<div style={{width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:48}}>🍽️</div>}
...
export default function App(){
  ...
  const [recipes,setRecipes]=useState(()=>{try{return JSON.parse(localStorage.getItem("cocina_recipes")||"[]");}catch{return [];}});
  ...
  return(
    <div style={{display:"flex",height:"100vh",width:"100vw",fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#F8F7F4",overflow:"hidden"}}>
      ...
      <div style={{flex:1,overflowY:"auto",minWidth:0}}>
        {page==="recetas"&&<RecipesPage recipes={recipes} onAdd={addRecipe} onDelete={deleteRecipe} onUpdate={updateRecipe} weekMenu={weekMenu} setWeekMenu={setWeekMenu} currentWeekOffset={currentWeekOffset} apiKey={apiKey} onNeedKey={()=>setApiKeyOpen(true)}/>}
...
with open("src/App.jsx","w",encoding="utf-8") as f:
    f.write(jsx)
print("OK: App.jsx v2 escrito correctamente")
