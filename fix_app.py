...
# Ejecuta este script desde la raíz del proyecto cocina-casa:
# python3 fix_app.py
...
function guessCategory(name){
  ...
  if(/tomate|cebolla|ajo|pimiento|patata|zanahoria|lechuga|espinaca|berenjena|calabacín|puerro|apio|pepino|brócoli|coliflor|alcachofa|verdura/.test(n))return"verduras";
  if(/arroz|pasta|fideos|garbanz|lenteja|judía|alubia|harina|pan|maíz|cereal|quinoa/.test(n))return"cereales";
  if(/tomate frito|salsa|conserva|lata|bote|caldo|aceitunas|alcaparra/.test(n))return"conservas";
...
function AddRecipeModal({open,onClose,onAdd}){
  ...
  async function callClaude(prompt,imgData=null){
    ...
    const res=await fetch("https://api.anthropic.com/v1/messages",{
      ...
      body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,
        system:"Eres un asistente de cocina. Devuelve SOLO JSON válido sin backticks ni texto adicional.",
        messages:msgs})
    });
  ...
  async function importRecipe(promptText,imgData=null){
    ...
    try{
      ...
      onClose();
    }catch(e){setError("Error al importar. Intenta de nuevo.");}
    setLoading(false);
  }
  ...
  function handleImport(){
    if(tab==="enlace"||tab==="video"){
      if(!url.trim())return;
      importRecipe(`Analiza esta URL de receta: ${url}\nExtrae toda la información y devuelve JSON:\n{"title":"","description":"","image":"","mealType":"Comida","recipeType":"Otros platos","ingredients":[{"amount":"","unit":"","name":""}],"steps":"","sourceUrl":"${url}","time":"","servings":4}`);
    }else if(tab==="buscar"){
      if(!searchQuery.trim())return;
      importRecipe(`Crea una receta detallada para: ${searchQuery}\nDevuelve JSON:\n{"title":"","description":"","image":"","mealType":"Comida","recipeType":"Otros platos","ingredients":[{"amount":"","unit":"","name":""}],"steps":"","sourceUrl":"","time":"","servings":4}`);
    }else if(tab==="texto"){
      if(!textInput.trim())return;
      importRecipe(`Analiza este texto de receta:\n${textInput}\nDevuelve JSON:\n{"title":"","description":"","image":"","mealType":"Comida","recipeType":"Otros platos","ingredients":[{"amount":"","unit":"","name":""}],"steps":"","sourceUrl":"","time":"","servings":4}`);
    }
  }
  ...
  function handleImage(e){
    ...
    reader.onload=()=>{
      ...
      importRecipe(`Analiza esta imagen de receta y extrae toda la información.\nDevuelve JSON:\n{"title":"","description":"","image":"","mealType":"Comida","recipeType":"Otros platos","ingredients":[{"amount":"","unit":"","name":""}],"steps":"","sourceUrl":"","time":"","servings":4}`,
        {type:file.type,data:base64});
    };
    reader.readAsDataURL(file);
  }

  const tabs=[{id:"foto",label:"📷 Foto"},{id:"enlace",label:"🔗 Enlace"},{id:"video",label:"🎥 Vídeo"},{id:"buscar",label:"🔍 Buscar"},{id:"texto",label:"📝 Texto"}];
  return(
    <Modal open={open} onClose={onClose} title="Añadir Receta" width={520}>
      ...
      {tab==="foto"&&(
        <div>
          <p style={{color:"#6B7280",fontSize:14,marginBottom:12}}>Sube una foto y la IA extraerá la receta.</p>
      ...
      {(tab==="enlace"||tab==="video")&&(
        <div>
          <p style={{color:"#6B7280",fontSize:14,marginBottom:12}}>Pega el enlace y la IA importará la receta automáticamente.</p>
      ...
      {tab==="buscar"&&(
        <div>
          ...
          <input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Ej: Paella valenciana..."
            style={{display:"block",width:"100%",marginTop:8,padding:"12px 14px",borderRadius:10,border:"1.5px solid #E5E7EB",fontSize:14,outline:"none",boxSizing:"border-box"}}
      ...
      {tab==="texto"&&(
        <div>
          <p style={{color:"#6B7280",fontSize:14,marginBottom:12}}>Pega el texto de la receta.</p>
      ...
      <button disabled={loading} onClick={handleImport}
        style={{marginTop:20,width:"100%",padding:"14px",background:loading?"#FED7AA":"#F97316",color:"#fff",border:"none",borderRadius:12,fontWeight:700,fontSize:15,cursor:loading?"not-allowed":"pointer"}}>
        ...
        {local.map(ing=>(
          <div key={ing.id} style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            <button onClick={()=>setLocal(prev=>prev.filter(i=>i.id!==ing.id))}
              style={{padding:"6px 10px",background:"#FEE2E2",border:"none",borderRadius:8,cursor:"pointer",color:"#EF4444"}}>🗑</button>
          </div>
        ))}
      </div>
      <button onClick={()=>setLocal(prev=>[...prev,{id:Date.now(),amount:"",unit:"unidad",name:""}])}
  ...
  return(
    <div style={{maxWidth:700,margin:"0 auto",padding:"20px 16px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
        ...
        <button onClick={()=>onDelete(recipe.id)} style={{background:"#FEE2E2",border:"none",borderRadius:8,padding:"8px 12px",cursor:"pointer",color:"#EF4444",fontSize:16}}>🗑️</button>
      </div>
      {recipe.image&&(
      ...
      {recipe.steps&&(
        <div>
          ...
          <div style={{fontSize:14,color:"#374151",lineHeight:1.8,whiteSpace:"pre-line",background:"#F9FAFB",padding:"16px",borderRadius:12}}>{recipe.steps}</div>
        </div>
      )}
      ...
      <AddToMenuModal open={addMenuOpen} onClose={()=>setAddMenuOpen(false)}
        recipe={recipe} weekMenu={weekMenu} setWeekMenu={setWeekMenu} currentWeekOffset={currentWeekOffset}/>
    </div>
  );
}
...
function RecipesPage({recipes,onAdd,onDelete,onUpdate,weekMenu,setWeekMenu,currentWeekOffset}){
  ...
  if(detail){
    ...
    return<RecipeDetail recipe={live} onBack={()=>setDetail(null)}
      ...
      onUpdate={onUpdate} weekMenu={weekMenu} setWeekMenu={setWeekMenu} currentWeekOffset={currentWeekOffset}/>;
  }

  const filtered=recipes.filter(r=>{
    const mS=r.title.toLowerCase().includes(search.toLowerCase());
    const mM=filterMeal==="Todas"||r.mealType===filterMeal;
    const mT=filterType==="Todos los tipos"||r.recipeType===filterType;
    return mS&&mM&&mT;
  });
  ...
  return(
    <div style={{padding:"28px 24px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:18}}>
        {filtered.map(r=>(
          <RecipeCard key={r.id} recipe={r} onOpen={setDetail} onDelete={onDelete} onAddMenu={setAddMenuRecipe}/>
        ))}
        {filtered.length===0&&(
...
function WeeklyMenuPage({recipes,weekMenu,setWeekMenu}){
  ...
  function buildWhatsApp(){
    ...
    DAYS.forEach(day=>{
      const slots=menu[day];if(!slots)return;
      txt+=`*${day}*\n`;
  ...
  return(
    <div style={{padding:"28px 24px"}}>
      ...
      <Modal open={recipePickerOpen} onClose={()=>{setRecipePickerOpen(false);setAddRecipeModal(null);}}
        title={`Añadir - ${addRecipeModal?.day} ${addRecipeModal?.slot}`} width={480}>
...
function ShoppingListPage({weekMenu,recipes}){
    ...
    Object.values(slots).forEach(rs=>{
      rs.forEach(r=>{
        ...
        (full?.ingredients||r.ingredients||[]).forEach(ing=>{
          ...
          if(!ingredientMap[k])ingredientMap[k]={...ing,id:k,category:guessCategory(ing.name)};
        });
      });
    });
  });
  ...
  function buildWhatsApp(){
    ...
    SHOPPING_CATEGORIES.forEach(c=>{
      const items=grouped[c.id];if(!items.length)return;
      txt+=`*${c.emoji} ${c.label}*\n`;
    ...
    <div style={{padding:"28px 24px"}}>
      ...
      {checkedCount>0&&(
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,padding:"12px 16px",background:"#F0FDF4",borderRadius:10}}>
          ...
          <button onClick={()=>{
            ...
            setExtraItems(p=>p.filter(i=>!ids.includes(i.id)));
      ...
      <div style={{display:"flex",gap:10,marginBottom:24}}>
        <input value={newItem} onChange={e=>setNewItem(e.target.value)} placeholder="Añadir alimento..."
          onKeyDown={e=>{if(e.key==="Enter"&&newItem.trim()){setExtraItems(p=>[...p,{id:`extra-${Date.now()}`,amount:"",unit:"",name:newItem.trim(),category:guessCategory(newItem)}]);setNewItem("");}}}
          style={{flex:1,padding:"12px 16px",borderRadius:12,border:"1.5px solid #E5E7EB",fontSize:14,outline:"none"}}/>
        <button onClick={()=>{if(!newItem.trim())return;setExtraItems(p=>[...p,{id:`extra-${Date.now()}`,amount:"",unit:"",name:newItem.trim(),category:guessCategory(newItem)}]);setNewItem("");}}
              ...
              {items.map((item,idx)=>(
                <div key={item.id}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px"}}>
                    ...
                    {checked[item.id]
                      ?<button onClick={()=>{setExtraItems(p=>p.filter(i=>i.id!==item.id));setChecked(p=>{const n={...p};delete n[item.id];return n;});}}
        ...
        {editItem&&(
          <>
            ...
            <div style={{display:"flex",gap:10}}>
              ...
              <button onClick={()=>{setExtraItems(prev=>{const idx=prev.findIndex(i=>i.id===editItem.id);if(idx>=0){const next=[...prev];next[idx]=editItem;return next;}return prev;});setEditItem(null);}}
  ...
  return(
    <div style={{display:"flex",height:"100vh",fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#F8F7F4"}}>
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        {page==="recetas"&&<RecipesPage recipes={recipes} onAdd={r=>setRecipes(p=>[r,...p])} onDelete={id=>setRecipes(p=>p.filter(r=>r.id!==id))} onUpdate={r=>setRecipes(p=>p.map(x=>x.id===r.id?r:x))} weekMenu={weekMenu} setWeekMenu={setWeekMenu} currentWeekOffset={currentWeekOffset}/>}
...
print(f"✅ {target} creado correctamente ({len(content)} caracteres)")
